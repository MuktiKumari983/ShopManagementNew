...
$stock_items = $conn->query("SELECT s.*, c.unit FROM stock s JOIN categories c ON s.category_id = c.id WHERE s.quantity > 0");
?>
<!DOCTYPE html>
<html>
<head>
...
<body class="p-4">
    <div class="container">
        ...
        <form method="POST" action="save_bill.php" id="billForm">
            <div class="row mb-3">
                <div class="col-md-4">
                    <label>Bill No</label>
                    <input type="text" name="bill_no" class="form-control" value="<?php echo $bill_no; ?>" readonly>
                </div>
                <div class="col-md-4">
            ...
            <table class="table" id="itemsTable">
                <thead>
                    <tr><th>Item</th><th>Quantity</th><th>Price</th><th>Total</th><th></th></tr>
                </thead>
                <tbody id="itemsBody">
                    <tr>
                        <td>
                            <select name="stock_id[]" class="form-control stock-select" required>
                                ...
                                ?>
                                <option value="<?php echo $item['id']; ?>" data-price="<?php echo $item['price']; ?>" data-stock="<?php echo $item['quantity']; ?>" data-unit="<?php echo $item['unit']; ?>">
                                    <?php echo $item['name']; ?> (Stock: <?php echo $item['quantity']; ?> <?php echo $item['unit']; ?>)
                                </option>
                                <?php endwhile; ?>
                            </select>
                        </td>
